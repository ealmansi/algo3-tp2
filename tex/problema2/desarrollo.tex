...
Dado que los pueblos se pueden identificar según su índice, y que las tuberías son vínculos no dirigidos entre dos pueblos, resulta natural representar un plan de construcción como el grafo $B(V, E)$ donde $V = \{1,\;2,\;...,\;n\}$ y el par $(i,j) \in E$ sii se construye una tubería entre $p_i$ y $p_j$. Se debe definir también un conjunto $C \subseteq V$, $|C| \leq k$, incluyendo los índices de aquellos pueblos donde se construyen centrales.

A las aristas de dicho grafo asignamos una función de peso $w:E\rightarrow\mathbb{R}$, determinando la longitud de las tuberías que representan, o concretamente:
$$si\;e = (i,j),\;w(e) = dist(p_i,p_j)$$

donde $dist$ representa la distancia euclidiana $\sqrt{(x_i - x_j)^2 + (y_i - y_j)^2}$.
...
\item Dada una solución con i componentes conexas (siendo i $<$ k), existe una solución mejor o igual con i+1 componentes conexas.
\end{itemize}
