...
\State Tipo de dato Matriz3D es vector $\langle$ vector $\langle$ vector $\langle$ Tupla $\langle$ Casillero, k $\rangle$ $\rangle$ $\rangle$ $\rangle$
    \Procedure{resolver}{$Entrada E$}
		...
		\While{!q.empty()} \Ode{1}
			...
			\For{cada Casillero c en losAdyacentes(e,c)} \Ode{n}
				\State u $\leftarrow$ siguienteAdyacente(c) \Ode{1}
        		\If{!estaEn(u,V)} \Ode{1}
		...
		\EndWhile
      	...
      	\Comment{Terminamos BFS. Escribimos Salida}
      	\State escriboSalida(S) \Ode{n}
...
\begin{pseudo}
    \Procedure{imprimoSalida}{$Salida s$}
